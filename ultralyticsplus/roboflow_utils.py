...
def extract_roboflow_metadata(url: str) -> tuple:
    match = re.search(
        r"https://(?:app|universe)\.roboflow\.com/([^/]+)/([^/]+)(?:/dataset)?/([^/]+)",
        url,
    )
    if match:
        ...
        raise ValueError(
            f"Invalid Roboflow dataset url ❌ "
            f"Expected: https://universe.roboflow.com/workspace_name/project_name/project_version. "
            f"Given: {url}"
        )


def push_to_roboflow_universe(exp_dir: str, roboflow_url: str, roboflow_token: str):
    workspace_name, project_name, project_version = extract_roboflow_metadata(
        url=roboflow_url
    )
    rf = Roboflow(api_key=roboflow_token)
    project_version = (
        rf.workspace(workspace_name).project(project_name).version(int(project_version))
    )
    LOGGER.info(
        f"Uploading model from local: {exp_dir} to Roboflow: {project_version.id}"
    )
